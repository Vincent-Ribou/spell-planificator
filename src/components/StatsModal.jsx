    ...
    >
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          ...
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: T.textDim, fontSize: 18, cursor: 'pointer', lineHeight: 1, padding: 0 }}>✕</button>
        </div>
        ...
        {STATS_CONFIG.map(({ key, label, description }) => (
          <div key={key} style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 4 }}>
              <label style={{ color: T.gold, fontSize: 13, fontWeight: 'bold', fontFamily: "'Cinzel', serif" }}>{label}</label>
              <span style={{ color: T.textDim, fontSize: 11, fontStyle: 'italic' }}>{description}</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <button
                onClick={() => setDraft(d => ({ ...d, [key]: String(Math.max(0, (parseInt(d[key]) || 0) - 1)) }))}
                style={{ width: 32, height: 32, background: T.bgInput, border: `1px solid ${T.border}`, borderRadius: 4, color: T.textSecondary, fontSize: 16, cursor: 'pointer', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}
              >−</button>
              <input
                type="number"
                min={0}
                ...
                style={{
                  ...
                  color: T.textPrimary, fontSize: 16, fontWeight: 'bold', fontFamily: "'Lora', serif",
                }}
              />
              <button
        ...
        <div style={{ borderTop: `1px solid ${T.border}`, marginTop: 20, marginBottom: 16 }} />

        <div style={{ display: 'flex', gap: 8 }}>
          <button
            onClick={handleReset}
            style={{ flex: 1, padding: '8px 0', background: T.bgInput, border: `1px solid ${T.border}`, borderRadius: 4, color: T.textMuted, fontSize: 13, cursor: 'pointer', fontFamily: "'Lora', serif" }}
          >
            Réinitialiser
          </button>
          <button
            onClick={handleSave}
            style={{ flex: 2, padding: '8px 0', background: T.saveBg, border: `1px solid ${T.saveBorder}`, borderRadius: 4, color: T.saveText, fontSize: 13, fontWeight: 'bold', cursor: 'pointer', fontFamily: "'Cinzel', serif", letterSpacing: '0.03em' }}
          >
            Sauvegarder
          </button>
        </div>
